...
%初始化DRNC(控制器)
Nc=6;pc=3;qc=1;
wc_in1=zeros(Nc,L);wc_in1(:,1)=0.2*ones(Nc,1);
wc_in2=zeros(Nc,L);wc_in2(:,1)=0.2*ones(Nc,1);
wc_in3=zeros(Nc,L);wc_in3(:,1)=0.2*ones(Nc,1);
wc_r=zeros(Nc,L);wc_r(:,1)=0.2*ones(Nc,1);
wc_out=zeros(Nc,L);wc_out(:,1)=0.2*ones(Nc,1);
...
nLr=[ones(L/4,1);zeros(L/4,1);ones(L/4,1);zeros(L/4,1)];     %目标转速
nL=zeros(L,1);      %实际低压转速
nH=zeros(L,1);      %实际高压转速
...
for k=2:1:L
    ...
    %DRNI更新
    Hi(:,k)=wi_in1(:,k-1)*wf(k)+wi_in2(:,k-1)*nL(k-1)+wi_r(:,k-1).*Si(:,k-1)+bi(:,k-1);
    Si(:,k)=Acfun(Hi(:,k));
    ...
    Q3c(:,k)=diffSc(:,k).*(nL(k-1)+wc_r(:,k-1).*Q3c(:,k-1));
    gradWc(p2c+1:p3c,k)=ec(k)*yu(k)*wc_out(:,k-1).*Q3c(:,k);       %第三个输入权值梯度
    Q2c(:,k)=diffSc(:,k).*(wf(k-1)+wc_r(:,k-1).*Q2c(:,k-1));
    gradWc(p1c+1:p2c,k)=ec(k)*yu(k)*wc_out(:,k-1).*Q2c(:,k);       %第二个输入权值梯度
    Q1c(:,k)=diffSc(:,k).*(nLr(k)+wc_r(:,k-1).*Q1c(:,k-1)); 
    ...
    Q2i(:,k)=diffSi(:,k).*(nL(k-1)+wi_r(:,k-1).*Q2i(:,k-1));
    gradWi(p1i+1:p2i,k)=em(k)*wi_out(:,k-1).*Q2i(:,k);       %第二个输入权值梯度
    Q1i(:,k)=diffSi(:,k).*(wf(k)+wi_r(:,k-1).*Q1i(:,k-1)); 
